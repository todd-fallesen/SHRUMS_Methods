...
% %added above, be sure to add them here auch.
    ...
    for k = 1:length(All_tracked_objects)
        table_for_tracked_object = Small_Table(Small_Table.TrackObjects_Label_2 == All_tracked_objects(k),:);
                                    ...
                                    total_intensity(k,2) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C5);  %brightfield                            
                                    ...
                                    total_intensity(k,8) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C2_enhanced);
                                    total_intensity(k,9) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C2_Enhanced_Corr);
                                    total_intensity(k,10) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C2_Corr);
                                    total_intensity(k,11) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C3);
                                    total_intensity(k,12) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C3_Enhanced);
                                    total_intensity(k,13) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C3_Enhanced_Corr);
                                    total_intensity(k,14) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C3_Corr);
                                    total_intensity(k,15) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C4);
                                    total_intensity(k,16) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C4_Enhanced);
                                    total_intensity(k,17) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C4_Corr);
                                    total_intensity(k,18) = sum(table_for_tracked_object.Intensity_IntegratedIntensity_C4_Enhanced_Corr);
                                    total_intensity(k,19) = max(table_for_tracked_object.AreaShape_Eccentricity);
                                    total_intensity(k,20) = sum(table_for_tracked_object.AreaShape_Area);
                                    total_intensity(k,21) = max(table_for_tracked_object.TrackObjects_Lifetime_2'); %make a new table with just the columns that we need
    end
